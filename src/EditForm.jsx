...
EditForm.propTypes = {
  ...
  editEntries: PropTypes.arrayOf(
    PropTypes.shape({
      ...
      type: EditEntryType.isRequired,
      isRequired: PropTypes.bool.isRequired,
      validations: PropTypes.arrayOf(ValidationType).isRequired,
      extraParam: PropTypes.any,
    })
  ).isRequired,
  entityObj: PropTypes.any.isRequired,  // Assuming `entityObj` can be of any type
...
export function EditForm(props) {
  ...
  const [pillList, setPillList] = useState(props.entityObj?.pillList || []);
  const [newPill, setNewPill] = useState("");
  const handleAddPill = () => {
    newPill && setPillList([...pillList, newPill]);
    newPill && entity['pillList'].push(newPill)
    newPill && setNewPill("")
    console.log(newPill)
    console.log(entity)
  };
  
  const handleRemovePill = (pillIndex) => {
    const updatedPills = pillList.filter((_, idx) => idx !== pillIndex);
    if (updatedPills) entity['pillList'] = updatedPills
          ...
          for (const editEntry of props.editEntries) {
            ...
            if (editEntry.isRequired) {
              if (!entity[editEntry.attribute]) {
                console.log(entity)
                console.log(entity[editEntry.attribute])
            ...
            if (editEntry.type === EditEntryType.Showcase) {
              ...
              const numPhotos = showcase.image_urls.length;
                  ...
                  if (editEntry.type === EditEntryType.PillList) {
                    return (
                      <div key={index} className="col-span-6 sm:col-span-3">
                        ...
                        <div className="my-2 flex flex-wrap gap-2" style={{display:'flex', gap:'4px', border:'0.5px solid grey', padding:'2px', borderRadius:'2.5px', height:'24.5px'}} >
                          {pillList.map((pill, pillIndex) => (
                            <div key={pillIndex} style={{color:'blue', width:'fit-content', border: '0.5px solid grey', borderRadius:'2.5px', paddingLeft:'2px' }} className="flex items-center bg-blue-200 text-blue-800 rounded-full px-3 py-1">
                              <span style={{fontFamily:'arial'}} >{pill}</span>
                              <button
                                type="button"
                                onClick={() => handleRemovePill(pillIndex)}
                                className="ml-2 text-xs text-red-500"
                                style={{background:'none', border:'none', color:'grey', lineHeight:'10px', fontSize:'20px'}}
                              >
                                x
                              </button>
                            </div>
                        ...
                        <input
                            ...
                            type="text"
                            //autoComplete={editEntry.attribute}
                            placeholder={"Enter new pill tag"}
                            value={newPill}
                        ...
                        <div className="flex mt-4">
                          <button
                            ...
                            onClick={() => {
                              //const newPill = prompt("Enter a new tag:"); // You can replace this with a more controlled input
                              //if (newPill) 
                                handleAddPill();
                            }}
                          >
                          ...
                          <button
                            ...
                            onClick={() => {
                              // Optionally remove the last pill if needed
                              handleRemovePill(pillList.length - 1);
                            }}
                          >
                            Remove
                          </button>
